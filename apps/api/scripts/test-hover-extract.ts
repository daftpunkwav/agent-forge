...
} from '../src/lib/llm/agentPrompt.ts';
...
for (const c of cases) {
  ...
  const dirtyPass =
    ...
    (isCompleteHoverAnswer(c.text) ||
      (isLikelyHoverTeaching(c.text) && !looksLikeHoverPlanning(c.text) && isCompleteHoverAnswer(c.text)));
  ...
  if (!pass || dirtyPass) {
    ...
    console.error(`FAIL ${c.name}`, {
      ...
      leak,
      missing,
      completeRaw: isCompleteHoverAnswer(c.text),
      dirtyPass,
    });
  } else {
    console.log(`PASS ${c.name}`, extracted ? `→ ${extracted.slice(0, 72)}…` : '→ empty');
  }
}
