...
import type { ByokConfig, LlmResponse, ProviderConfig, StreamChunk } from '../lib/llm/types.js';
...
import { setHoverCache } from '../services/hoverCache.js';
...
function hoverCacheJson(body: ExplainBody, style: string, cached: string) {
  return {
    explanation: cached,
    mode: body.mode,
    model: 'cache',
...
function sseWriteHoverCache(res: Parameters<typeof sseWrite>[0], body: ExplainBody, style: string, cached: string) {
  sseWrite(res, {
    type: 'meta',
    model: 'cache',
    format: 'cache',
...
agentRouter.post(
  ...
  async (req, res, _next) => {
    ...
    try {
      ...
      const resolved = await resolveStreamWithFallback(
        {
          mode: prep.isHover ? 'fast' : 'deep',
          maxTokens: prep.isHover ? 220 : 2048,
          temperature: prep.isHover ? 0.15 : undefined,
