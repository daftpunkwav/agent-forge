...
> 前置报告：`docs/architecture-review-2026-08-04.md`（结构/安全/文档一致性）；本报告聚焦**故障隔离与韧性**，与其互补不重复
...
> 简化实现建议（写进 TODO 注释）：流式 failover 第一版只覆盖「熔断 503」一种情况，约 20 行；其余场景留待 tool-loop 原生 tools API 化时一并重构（`docs/tool-loop-roadmap.md`）。
...
**扩容判断标准**（写进运维文档）：单进程 event loop 延迟 p99 >100ms、或 CPU 持续 >70%、或 `/ready` 抖动 → 先垂直扩容；再不够 → Postgres（`docs/postgres.md`）+ 多副本 + Redis store。
...
| 阶段 2（单进程 + PG） | DB 锁竞争、`/ready` 抖动 | 按 `docs/postgres.md` 切 PostgreSQL；`DATABASE_URL` 加 `connection_limit=10` |
