...
> 关系：本文为 `docs/comprehensive-review-2026-08-04.md` 的**增量审查**，编号前缀 `R2-`，避免与 R1 的 `SEC/ARC/WEB/Q/DOC` 重复。
...
- `docs/httponly-cookie-migration.md` 已识别需要 HttpOnly 迁移，但**未**配套 CSRF 防护。
...
- `docs/postgres.md` 未声明这一行为差异；schema 中 tags/summary 字段无 `pg_trgm` 索引。
...
- 在 `docs/postgres.md` 增加 "PostgreSQL 与 SQLite 的查询行为差异" 章节，提供索引迁移 SQL。
...
1. **R2-09** 切 PG 前的 `contains` 行为差异与 `pg_trgm` + GIN 索引；`docs/postgres.md` 增补。
