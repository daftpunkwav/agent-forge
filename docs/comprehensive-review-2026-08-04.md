...
> 审查日期：2026-08-04  
> 审查基准：仓库当前工作树；代码基准 commit `224cfdb` 及其后仅存在的既有未跟踪文件 `docs/architecture-review-2026-08-04.md`  
...
- 仓库已有 `docs/httponly-cookie-migration.md`，说明这是已识别但尚未落地的设计项。
...
`docs/architecture.md` 声称 `_legacy` 已被忽略、Annotation 尚无 API 路由；当前 git 与 `apps/api/src/routes/annotations.ts` 均显示相反事实。
...
1. 按 `docs/httponly-cookie-migration.md` 迁移 refresh token 到 HttpOnly cookie，并加入 Origin/CSRF 策略。
