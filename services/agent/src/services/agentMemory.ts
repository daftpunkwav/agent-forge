 ...
 * 工厂注入 PrismaClient + UserQueryPort(偏好)+ ArticleQueryPort(进度标题)。
 * 边界：不直接查 user / article 表——偏好走 users 端口，文章标题走 articles 端口。
 */
...
export function createAgentMemory(
   ...
   * loadUserContext 每次执行多条查询 + BYOK 解密，而记忆/进度在 60s 内几乎不变。
  ...
  async function loadUserContextInner(userId?: string, route?: string) {
    if (!userId) {
      return {
        ...
        byok: null as ReturnType<typeof decryptByokConfig>,
      };
    }
    ...
    // A-03：库中为密文（identity 返回密文），读取时解密供 llm 网关使用
    const byok = decryptByokConfig(prefs?.byok ?? null);
    ...
    // 跨服务补标题：progress 只存 articleId，文章元信息走 content 端口
